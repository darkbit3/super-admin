...
// ── Create Group Modal (already a proper top-level component) ───────────────

function CreateGroupModal({ people, onClose, onCreate }) {
  const [name, setName]                   = useState('')
  const [description, setDescription]     = useState('')
  const [selectedMemberIds, setSelected]  = useState([])
  const [submitting, setSubmitting]       = useState(false)
  const [filterQuery, setFilterQuery]     = useState('')

  const selectablePeople = (people || []).filter((p) => !p.isSuperAdmin && p.id)
  ...
  const handleSubmit = async () => {
    ...
    if (selectedMemberIds.length === 0) { alert('Select at least one member'); return }
    try {
      setSubmitting(true)
      const res = await api.post('/chat/groups', {
        name: name.trim(), description: description.trim(), memberIds: selectedMemberIds,
      })
      ...
      <div className="w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl bg-white border border-purple-100 flex flex-col max-h-[85vh]">
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Operations Team"
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-violet-100 text-violet-700">{selectedMemberIds.length} selected</span>
            </div>
            ...
            <div className="rounded-xl border border-purple-100 max-h-52 overflow-y-auto p-1.5 bg-slate-50/40 space-y-1">
              {filteredMembers.length === 0 ? (
                ...
                return (
                  <button key={person.id} type="button" onClick={() => toggleMember(person.id)}
                    className={`flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2 text-left transition-colors ${isSelected ? 'bg-violet-100/70' : 'hover:bg-slate-100/70'}`}>
                    ...
                    <div className={`w-5 h-5 rounded-md flex items-center justify-center border transition-all ${isSelected ? 'bg-violet-600 border-violet-600 text-white' : 'border-slate-300 bg-white'}`}>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-purple-100 px-6 py-3.5 bg-slate-50/60">
          <button type="button" onClick={onClose}
            className="rounded-xl px-4 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-200/70 transition-colors">Cancel</button>
...
// ── ConversationPanel — owns draft state so typing never re-renders parent ──
const ConversationPanel = memo(function ConversationPanel({
  ...
  // draft lives HERE — typing only re-renders this component, not Chat
          ...
          <div className="min-w-0">
            ...
            {selectedGroup ? (
              <div className="mt-0.5 flex items-center gap-2 text-[11px] text-slate-500">
                <span className="px-2 rounded-full bg-violet-100 text-violet-700 font-semibold text-[10px]">Group</span>
        ...
        {groups.length > 0 && (
          <div>
            <div className="px-4 pt-3 pb-1 flex items-center justify-between">
              ...
              <span className="text-[10px] font-semibold px-1.5 rounded-full bg-emerald-100 text-emerald-800">{groups.length}</span>
            </div>
            {groups.map((g) => (
...
export default function Chat() {
  ...
  const handleCreateGroup = useCallback(() => setShowGroupModal(true),  [])
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold font-heading text-slate-900 tracking-tight">Portal Chat</h1>
          </div>
