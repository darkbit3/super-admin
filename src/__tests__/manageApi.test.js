...
describe('super-admin management API', () => {
  it('sends the admin creation payload to the management endpoint', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith('https://backend-1-naba.onrender.com/api/super/admins', expect.objectContaining({
      method: 'POST',
  ...
  it('sends bulk status updates with the selected IDs and status', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith('https://backend-1-naba.onrender.com/api/super/admins/bulk/status', expect.objectContaining({
      method: 'POST',
